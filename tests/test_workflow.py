...
def load_inputs(path):
    with open(path, newline="") as f: return {(row.get("input") or row.get("smiles", "")).strip() for row in csv.DictReader(f)} - {""}
...
def test_1_write(cfg, state):
    cli("write", "-m", cfg["model"], "-v", cfg["version"], "-pn", cfg["src"], "--access", "public", "-i", cfg["input"])
    state["wrote"] = True

def test_2_read(cfg, state):
    if not state.get("wrote"): pytest.skip("write did not pass")
...
def test_3_read_batched(cfg, state):
    ...
    reader = IsauraReader(model_id=cfg["model"], model_version=cfg["version"], bucket=cfg["src"], input_csv=cfg["input"], approximate=False)
...
def test_7_remove_model(cfg, state):
    if not (state.get("copied") or state.get("moved")): pytest.skip("copy/move did not pass")
